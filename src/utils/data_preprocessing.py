...
def fitAndSaveTokenizer(text_sentences):
    ...
    print(f"Fitting Tokenier with with VocabSize - {ModelConfigs.VOCAB_SIZE}..\n")
    ...
    print(f"Tokenizer object saved at {FilePathConstants.TOKENIZER_SAVE_PATH}..\n")
...
def loadAndPreprocessData():
    ...
    train_data = pd.read_csv(FilePathConstants.PROCESSED_TRAIN_DATA_PATH, delimiter='\t')
    test_data = pd.read_csv(FilePathConstants.PROCESSED_TEST_DATA_PATH, delimiter='\t')
    ...
    if not os.path.isfile(FilePathConstants.TOKENIZER_SAVE_PATH):
        print(f"Saved tokenizer object not found in path - {FilePathConstants.TOKENIZER_SAVE_PATH}..\n")
        fitAndSaveTokenizer(X_train)
    
    print(f"Loading tokenizer object from {FilePathConstants.TOKENIZER_SAVE_PATH}\n")
...
def preprocessTexts(texts):
    if not isinstance(texts, Iterable):
        raise Exception("'texts' input is not iterable. Pass texts as an iterable object")
    
    if not os.path.isfile(FilePathConstants.TOKENIZER_SAVE_PATH):
        raise Exception(f"Saved tokenizer object not found in path - {FilePathConstants.TOKENIZER_SAVE_PATH}..\n")
